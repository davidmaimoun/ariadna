...
export default function PhyloTree({ data, onClose, autoLoad, onAutoLoaded }) {
  ...
  useEffect(() => {
    if (!autoLoad) return
    const { algoId, text, fileName } = autoLoad
    const type = algoId === 'nwk' ? 'newick' : 'matrix'
    const t = setTimeout(() => {
      handleSelectRef.current?.(algoId, text, type, fileName)
        .then(() => onAutoLoaded?.())
        .catch(console.error)
    }, 50)
    ...
    try {
      ...
      if (type==='newick') {
        ...
        if (algoId==='nj'||algoId==='upgma') {
          ...
          const newData = { tree:t, graph:null, filename:fname, algo:algoId, isAllelic:dm.isAllelic }
          setTreeData(newData)
          setMode('rect')
        } else {
          let g
          ...
          const newData2 = { tree:null, graph:g, filename:fname, algo:algoId, isAllelic:dm.isAllelic }

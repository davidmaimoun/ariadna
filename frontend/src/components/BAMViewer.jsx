...
export default function BAMViewer({ data, onClose, width, height }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#f0f5ff' }}>
      {/* Header */}
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 16px', background:'#1a3faa', flexShrink:0 }}>
        <span style={{ fontSize:18, fontWeight:800, color:'#fff', fontFamily:'"IBM Plex Sans",sans-serif' }}>BAM / SAM Viewer</span>
        <div style={{ display:'flex', gap:16 }}>
          {[['Reads',stats.total.toLocaleString()],['Mapped',stats.mapped.toLocaleString()],['Avg len',stats.avgLen+'bp'],['Avg MAPQ',stats.avgMapq]].map(([l,v])=>(
            <span key={l} style={{ fontSize:12, color:'#a0c0ff' }}><b style={{ color:'#fff' }}>{v}</b> {l}</span>
          ))}
        </div>
        {chroms.length > 1 && (
          <select value={chrom} onChange={e=>setChrom(e.target.value)} style={{ fontSize:12, background:'rgba(255,255,255,0.15)', color:'#fff', border:'1px solid rgba(255,255,255,0.3)', borderRadius:6, padding:'3px 8px' }}>
            {chroms.map(c=><option key={c} value={c} style={{ background:'#1a3faa' }}>{c} ({sq[c]?.toLocaleString()||'?'} bp)</option>)}
          </select>
        )}
        <button className="btn" style={{ marginLeft:'auto', background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', color:'#fff' }} onClick={onClose}>✕ Close</button>
      </div>

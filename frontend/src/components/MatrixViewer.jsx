...
export default function MatrixViewer({ data, onClose }) {
  ...
  const [viewMode,   setViewMode]   = useState('heatmap') // 'heatmap' | 'table' | 'pa'
  ...
  const filteredRows = useMemo(() => {
    if (!search) return rowOrder
    const q=search.toLowerCase()
    return rowOrder.filter(i=>rowLabels[i].toLowerCase().includes(q))
  },[rowOrder,rowLabels,search])

  const filteredCols = useMemo(() => {
    if (!search||!isSymmetric) return colOrder
    const q=search.toLowerCase()
    return colOrder.filter(j=>colLabels[j].toLowerCase().includes(q))
  },[colOrder,colLabels,search,isSymmetric])
  ...
  // Layout constants
  const LABEL_W  = 140  // row labels on left
  const LABEL_H  = 140  // col labels on top — enough room for 45° rotated text
  ...
  const draw = useCallback(() => {
    ...
    // ── Header bg — drawn AFTER cells to cover any overflow ─────────────────
    ctx.fillStyle = '#eef3ff'
    ctx.fillRect(0, 0, totalW, LABEL_H)    // full-width solid header
    ctx.fillStyle = '#f4f7ff'
    ctx.fillRect(0, 0, LABEL_W, totalH)    // left label column
    ...
    for (let ci = 0; ci < filteredCols.length; ci++) {
      ...
      ctx.rotate(-Math.PI / 2)             // -90°: +x = UP in screen
      ctx.textAlign = 'left'               // text starts at anchor → goes UP
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'var(--bg)', fontFamily:'"IBM Plex Sans",sans-serif' }}>
      {/* Header */}
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 14px', background:'#0f3fa8', flexShrink:0 }}>
        <span style={{ fontSize:15, fontWeight:800, color:'#fff' }}>📊 Matrix Viewer</span>
        <span style={{ fontSize:12, color:'#a0c0ff' }}>{n} × {nCols} · {isSymmetric?'distance matrix':'allelic profile'}</span>
        <div style={{ flex:1 }}/>
        <button className="btn" style={{ background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', color:'#fff', fontSize:12 }} onClick={exportPNG}><Download size={13}/> PNG</button>
        <button className="btn" style={{ background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', color:'#fff', fontSize:12 }} onClick={exportTSV}><Download size={13}/> TSV</button>
        <button className="btn" style={{ background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', color:'#fff', fontSize:12 }} onClick={onClose}><X size={13}/> Close</button>
      </div>

      {/* Controls */}
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'6px 14px', background:'var(--bg3)', borderBottom:'1px solid var(--border)', flexShrink:0, flexWrap:'wrap' }}>
        {/* View mode */}
        <div style={{ display:'flex', gap:4 }}>
          {[['heatmap','🎨 Heatmap'],['table','📋 Table'],['pa','⬜ Presence/Absence']].map(([id,label])=>(
            <button key={id} onClick={()=>setViewMode(id)}
              className={viewMode===id?'btn btn-primary':'btn'}
        ...
        {viewMode==='pa'&&(
          <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'var(--txt2)' }}>
            Threshold &gt;
            <input type="number" value={paThreshold} step="any"
              onChange={e=>setPaThreshold(parseFloat(e.target.value)||0)}
        ...
        {viewMode==='heatmap'&&(
          <div style={{ display:'flex', gap:5, alignItems:'center' }}>
            <span style={{ fontSize:12, color:'var(--txt3)' }}>Color:</span>
            {Object.keys(COLORSCALES).map(cs=>(
              <button key={cs} onClick={()=>setColorscale(cs)} title={cs} style={{
                ...
                background:`linear-gradient(to right,${COLORSCALES[cs](0)},${COLORSCALES[cs](0.5)},${COLORSCALES[cs](1)})`,
              }}/>
            ))}
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:4 }}>
          <span style={{ fontSize:12, color:'var(--txt3)', alignSelf:'center' }}>Sort:</span>
          {[['original','Original'],['sum','By sum'],['alpha','A→Z']].map(([id,label])=>(
            <button key={id} onClick={()=>setSortMode(id)} className={sortMode===id?'btn btn-primary':'btn'} style={{ padding:'4px 9px', fontSize:11 }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ position:'relative', display:'flex', alignItems:'center' }}>
          ...
          <input type="text" placeholder="Filter…" value={search} onChange={e=>setSearch(e.target.value)} style={{ width:140, paddingLeft:26, fontSize:12 }}/>
        </div>

        {viewMode!=='table'&&(
          <div style={{ display:'flex', alignItems:'center', gap:4 }}>
            <button className="btn-zoom" onClick={()=>setCellSize(s=>Math.max(2,(s||autoCell)-2))}><ZoomOut size={14}/></button>
            <span style={{ fontSize:11, color:'var(--txt3)', fontFamily:'monospace', minWidth:32, textAlign:'center' }}>{cell}px</span>
            <button className="btn-zoom" onClick={()=>setCellSize(s=>Math.min(64,(s||autoCell)+2))}><ZoomIn size={14}/></button>
            <button className="btn btn-ghost" style={{ fontSize:11 }} onClick={()=>setCellSize(null)}>Auto</button>
          </div>
        )}

        <div style={{ flex:1 }}/>
        <span style={{ fontSize:11, color:'var(--txt4)' }}>{filteredRows.length}×{filteredCols.length} · min {minVal.toFixed(1)} · max {maxVal.toFixed(1)}</span>
      </div>
      ...
      <div ref={containerRef} style={{ flex:1, overflow:'auto', background:'#fafcff', cursor:'crosshair' }}>
        {viewMode==='table' ? (
          /* HTML table view */
          <table style={{ borderCollapse:'collapse', fontSize:11, fontFamily:'"JetBrains Mono",monospace', whiteSpace:'nowrap' }}>
            <thead style={{ position:'sticky', top:0, background:'#f0f5ff', zIndex:2 }}>
              <tr>
                <th style={{ padding:'6px 10px', borderBottom:'2px solid var(--border)', textAlign:'left', fontSize:11, color:'var(--txt3)', minWidth:LABEL_W }}></th>
                {filteredCols.map(j=>(
                  <th key={j} style={{ padding:'4px 6px', borderBottom:'2px solid var(--border)', textAlign:'center', fontSize:10, color:'var(--txt2)', fontWeight:600, maxWidth:60, overflow:'hidden', textOverflow:'ellipsis' }}>
                    <div style={{ transform:'rotate(-45deg)', transformOrigin:'bottom left', whiteSpace:'nowrap', fontSize:9, maxWidth:80 }}>
                      {colLabels[j].slice(0,16)}
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filteredRows.map((i,ri)=>(
                <tr key={i} style={{ background:i===highlight?'#e4edfb':ri%2===0?'#fff':'#f8faff' }}
                  ...
                  <td style={{ padding:'4px 10px', fontWeight:600, color:rowLabels[i]===highlight?'#1a56db':'#0f2460', borderRight:'1px solid var(--border2)', position:'sticky', left:0, background:'inherit' }}>
                  ...
                  {filteredCols.map(j=>{
                    ...
                    return (
                      <td key={j} style={{
                        padding:'3px 6px', textAlign:'center', fontSize:11,

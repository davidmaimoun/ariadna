...
export default function VCFViewer({ data, onClose }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'#f0f5ff' }}>
      {/* Header */}
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 16px', background:'#1a3faa', borderBottom:'1px solid #0f2c80', flexShrink:0 }}>
        <span style={{ fontSize:18, fontWeight:800, color:'#fff', fontFamily:'"IBM Plex Sans",sans-serif' }}>VCF Viewer</span>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {Object.entries(TYPE_COLORS).map(([t, c]) => {
            const n = variants.filter(v=>v.type===t).length
            if (!n) return null
            return <span key={t} style={{ padding:'2px 10px', borderRadius:10, fontSize:11, fontWeight:700, background:c.light, color:c.text, border:`1px solid ${c.bg}` }}>{t} <b>{n}</b></span>
          })}
        </div>
        <span style={{ fontSize:12, color:'#a0c0ff', marginLeft:'auto' }}>{variants.length.toLocaleString()} variants total</span>
        <button className="btn" style={{ background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.3)', color:'#fff' }} onClick={onClose}>✕ Close</button>
      </div>

      {/* Filters */}
      <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 16px', background:'#dce8fb', borderBottom:'1px solid #c0d4f5', flexShrink:0, flexWrap:'wrap' }}>
        <select value={filterType} onChange={e=>{ setFilterType(e.target.value); setPage(0) }} style={{ fontSize:12 }}>
          {types.map(t => <option key={t} value={t}>{t}</option>)}
        </select>
        <label style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'#1a3faa', cursor:'pointer' }}>
          <input type="checkbox" checked={filterPass} onChange={e=>{ setFilterPass(e.target.checked); setPage(0) }} style={{ accentColor:'#1a56db' }} />
          PASS only
        </label>
        <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#1a3faa' }}>
          Min QUAL
          <input type="number" value={minQual} min={0} onChange={e=>{ setMinQual(parseInt(e.target.value)||0); setPage(0) }} style={{ width:64, fontSize:12 }} />
        </label>
        <div style={{ position:'relative' }}>
          <input type="text" placeholder="Search ID / chrom / allele…" value={search}
            onChange={e=>{ setSearch(e.target.value); setPage(0) }}
            style={{ width:220, paddingLeft:28, fontSize:12 }} />
          <svg width="12" height="12" viewBox="0 0 16 16" fill="#93b4f0" style={{ position:'absolute', left:8, top:'50%', transform:'translateY(-50%)', pointerEvents:'none' }}>
        ...
        <span style={{ fontSize:12, color:'#5a7ec0', marginLeft:'auto' }}>{filtered.length.toLocaleString()} shown</span>
      </div>
            ...
            {paged.map((v, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding:'5px 10px', color:'#0f2460' }}>{v.qual?.toFixed(1) ?? '—'}</td>

...
export default function SangerViewer({ files: initialFiles, onClose }) {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'var(--bg)',fontFamily:'"IBM Plex Sans",sans-serif'}}>
      {/* Header */}
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'7px 14px',background:'#1a3faa',flexShrink:0,height:50}}>
        <span style={{fontSize:15,fontWeight:800,color:'#fff'}}>🧫 Sanger Trace Viewer</span>
        <div style={{display:'flex',gap:5,overflowX:'auto',flex:1}}>
          {files.map((f,i)=>(
            <button key={i} onClick={()=>{setActive(i);setEditedBases(null)}} style={{
              padding:'3px 11px',borderRadius:6,border:'none',cursor:'pointer',
              fontSize:11.5,fontFamily:'"JetBrains Mono",monospace',whiteSpace:'nowrap',
              background:active===i?'#ffe000':'rgba(255,255,255,0.15)',
              color:active===i?'#0f2460':'#c0d8ff',fontWeight:active===i?700:400,transition:'all .15s',
            }}>{f.name.replace(/\.ab1$/i,'')}</button>
          ))}
        </div>
        <input ref={fileInputRef} type="file" accept=".ab1" multiple style={{display:'none'}} onChange={e=>loadFiles(e.target.files)}/>
        <button className="btn" style={{background:'rgba(255,255,255,0.15)',border:'1px solid rgba(255,255,255,0.3)',color:'#fff',fontSize:12}}
          onClick={()=>fileInputRef.current.click()}>+ Add .ab1</button>
        {data&&(
          <button className="btn" style={{background:'rgba(255,255,255,0.15)',border:'1px solid rgba(255,255,255,0.3)',color:'#fff',fontSize:12}}
            onClick={exportFASTA}><Download size={13}/> FASTA</button>
        )}
        <button className="btn" style={{background:'rgba(255,255,255,0.15)',border:'1px solid rgba(255,255,255,0.3)',color:'#fff',fontSize:12}}
          onClick={onClose}><X size={13}/> Close</button>
      </div>

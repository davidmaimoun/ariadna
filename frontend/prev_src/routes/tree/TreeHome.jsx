...
export default function TreeHome() {
  ...
  const pick = (algo, file) => {
    // The document stores the algo + raw file; PhyloTree computes on mount
    const id = addDoc('tree', file.name, { algoId: algo, file })
    navigate(`/tree/${id}`)
  }
          ...
          return (
            <div key={a.id} style={{ gridColumn: isNwk ? '1 / -1' : undefined }}>
              ...
              <button onClick={() => refs.current[a.id]?.click()}
                ...
                <div>
                  <div style={{ fontSize:12.5, fontWeight:700, color:'#0f2460', marginBottom:4 }}>{a.label}</div>
                  <div style={{ fontSize:11, color:'#5a7ec0', lineHeight:1.4 }}>{a.desc}</div>
                  <div style={{ fontSize:10, color:a.color, fontWeight:600, marginTop:5 }}>

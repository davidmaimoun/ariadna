...
import MSAToolbar from '../toolbars/MSAToolbar'
import MSASidebar from '../sidebars/MSASidebar'

/*
  MSAViewer — Multiple Sequence Alignment viewer
  Accepts aligned FASTA (.afa, .aln, .fa, .fasta with equal-length sequences)
  Shows: consensus, per-column conservation, aligned sequences with colored gaps
*/
...
export default function MSAViewer({ sequences: seqsProp, onClose }) {
  ...
  const handleWheel = (e) => {
    ...
    if (e.ctrlKey || e.metaKey) {
      // Ctrl+Wheel → zoom cell width
      setCellW(w => Math.max(4, Math.min(28, w + (e.deltaY < 0 ? 2 : -2))))
    } else if (e.shiftKey) {
      // Shift+Wheel → scroll sequences vertically
      const delta = e.deltaY > 0 ? 3 : -3
      setSeqOffset(s => Math.max(0, Math.min(seqs.length - visibleSeqs, s + delta)))
    } else {
      // Wheel → scroll columns horizontally
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f0f7f2' }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize:11, color:'#9dbfad', fontFamily:'monospace', fontWeight:600 }}>{seqType.toUpperCase()}</span>

        <div style={{ flex: 1 }} />

        {/* Zoom */}
        <span style={{ fontSize:11, color:'#6b9e82', fontWeight:600 }}>Zoom</span>
        {[6, 10, 14, 20].map(w => (
          <button key={w} onClick={() => setCellW(w)}
            style={{ padding:'4px 9px', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer',
              border:`1.5px solid ${cellW===w?'#1a7a40':'#b8d9c2'}`,
              background:cellW===w?'#1a7a40':'#fff', color:cellW===w?'#fff':'#3a7a5a', transition:'all .12s' }}>
            {w}px
          </button>
        ))}
        <button onClick={()=>setCellW(w=>Math.max(4,w-2))}
          style={{ width:28,height:28,borderRadius:7,border:'1.5px solid #b8d9c2',background:'#fff',cursor:'pointer',color:'#3a7a5a',fontSize:18,display:'flex',alignItems:'center',justifyContent:'center' }}>−</button>
        <button onClick={()=>setCellW(w=>Math.min(28,w+2))}
          style={{ width:28,height:28,borderRadius:7,border:'1.5px solid #b8d9c2',background:'#fff',cursor:'pointer',color:'#3a7a5a',fontSize:18,display:'flex',alignItems:'center',justifyContent:'center' }}>+</button>
        <div style={{ height: 16, borderLeft: '1px solid #b8d9c2' }} />
      ...
      <div style={{ padding: '3px 12px', background: '#e6f2ea', borderBottom: '1px solid #d4eadb', fontSize: 10, color: '#9dbfad' }}>
        Wheel → scroll columns · Shift+Wheel → scroll sequences · Ctrl+Wheel → zoom in/out
        {hoverCol !== null && (
